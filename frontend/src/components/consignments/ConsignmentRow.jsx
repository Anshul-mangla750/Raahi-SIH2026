...
export default function ConsignmentRow({ item, consignment, onViewDetails = () => {} }) {
  ...
  const defaultTimeline = [
    { step: 'Order Placed', status: 'completed', date: rowItem.bookedOn || 'Today' },
    { step: 'In Transit', status: rowItem.statusType === 'delivered' ? 'completed' : 'current', currentColor: 'blue', date: 'Active' },
    { step: 'Delivered', status: rowItem.statusType === 'delivered' ? 'completed' : 'upcoming', date: rowItem.etaDate || 'Pending' },
  ];
    ...
    <div className="bg-white rounded-2xl border border-slate-200/80 shadow-2xs hover:shadow-xs transition-all p-4 sm:p-5 flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4 sm:gap-6 overflow-hidden">
      ...
      <div className="flex flex-col justify-between flex-1 min-w-[220px] max-w-[280px]">
        ...
        <div className="flex items-center gap-2 my-2 text-xs font-bold text-slate-800">
          <div className="flex flex-col">
            <span>{origin}</span>
            <span className="text-[10px] text-slate-400 font-medium -mt-0.5">{originState}</span>
          </div>
          <ArrowRight className="w-3.5 h-3.5 text-slate-400 flex-shrink-0 mx-1" />
          <div className="flex flex-col">
            <span>{destination}</span>
            <span className="text-[10px] text-slate-400 font-medium -mt-0.5">{destinationState}</span>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 text-[10px] text-slate-500 font-bold">
          ...
          <span className={`px-1.5 py-0.5 rounded-md text-[9px] font-extrabold border ${getPriorityBadge(priorityType)}`}>
            {priority}
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-col justify-center min-w-[130px] border-t lg:border-t-0 lg:border-l border-slate-100 pt-3 lg:pt-0 lg:pl-5">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-extrabold text-slate-800">{cargoName}</span>
        </div>
